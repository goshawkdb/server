...
func (fo *frameOpen) ReadRetry(action *localAction) bool {
	...
	case fo.currentState != fo:
		...
		return false
	case fo.frameTxnActions == nil || fo.frameTxnId.Equal(action.readVsn):
		return false
	default:
...
func (fo *frameOpen) ReadLearnt(action *localAction) bool {
	...
	if !action.readVsn.Equal(fo.frameTxnId) {
...
func (fo *frameOpen) WriteLearnt(action *localAction) bool {
	...
	reqClockElem := fo.frameTxnClock.Clock[*fo.v.UUId]
	if actClockElem < reqClockElem || (actClockElem == reqClockElem && action.Id.LessThan(fo.frameTxnId)) {
...
func (fo *frameOpen) calculateReadVoteClock() {
	if fo.readVoteClock == nil {
		...
		for k, v := range clock.Clock {
			if m, found := fo.mask.Clock[k]; found && m >= v {
				delete(clock.Clock, k)
			}
		}
		for k, v := range written.Clock {
			if m, found := fo.mask.Clock[k]; k.Equal(fo.v.UUId) || !found || m < v {
				clock.SetVarIdMax(k, v+1)
			}
		}
		fo.readVoteClock = clock
		if _, found := fo.frameWritesClock.Clock[*fo.v.UUId]; !found {
...
func (fo *frameOpen) maybeCreateChild() {
	...
	for node := fo.writes.First(); node != nil; node = node.Next() {
		action := node.Key.(*localAction)
		localElemVal := action.outcomeClock.Clock[*vUUId]
	...
	elem := fo.frameTxnClock.Clock[*fo.v.UUId]
	switch {
