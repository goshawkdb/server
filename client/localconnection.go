...
func (msg *localConnectionMsgRunClientTxn) consumer(txn *eng.TxnReader, outcome *msgs.Outcome, err error) error {
	msg.txnReader = txn
	msg.outcome = outcome
	msg.err = err
	msg.MustClose()
...
func (msg *localConnectionMsgRunClientTxn) Exec() (bool, error) {
	...
	return false, msg.submitter.SubmitClientTransaction(msg.translationCallback, txn, txnId, msg.consumer, nil, msg.isTopologyTxn, nil)
}
...
func (msg *localConnectionMsgRunTxn) consumer(txn *eng.TxnReader, outcome *msgs.Outcome, err error) error {
	msg.txnReader = txn
	msg.outcome = outcome
	msg.err = err
	msg.MustClose()
...
func (msg *localConnectionMsgRunTxn) Exec() (bool, error) {
	...
	msg.submitter.SubmitTransaction(txn, txnId, msg.activeRMs, msg.consumer, msg.backoff)
	return false, nil
}
...
func NewLocalConnection(rmId common.RMId, bootCount uint32, cm paxos.ConnectionManager, logger log.Logger) *LocalConnection {
	...
	lci := &localConnectionInner{
		LocalConnection:  lc,
		BasicServerInner: actor.NewBasicServerInner(logger),
	}

	mailbox, err := actor.Spawn(lci)
	if err != nil {
		panic(err) // "impossible"
	}

	lc.BasicServerOuter = actor.NewBasicServerOuter(mailbox)
	lc.inner = lci
...
func (lc *localConnectionInner) Init(self *actor.Actor) (bool, error) {
	lc.LocalConnection.Mailbox = self.Mailbox

	servers, _ := lc.connectionManager.ClientEstablished(0, lc.LocalConnection)
	if servers == nil {
		panic("LocalConnection failed to register with ConnectionManager!")
	}
	topology := lc.connectionManager.AddTopologySubscriber(eng.ConnectionSubscriber, lc.LocalConnection)
	if err := lc.submitter.TopologyChanged(topology); err != nil {
		return false, err
	}
	if err := lc.submitter.ServerConnectionsChanged(servers); err != nil {
		return false, err
	}
	lc.connectionManager.AddServerConnectionSubscriber(lc.LocalConnection)
	return lc.BasicServerInner.Init(self)
}
